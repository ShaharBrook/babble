...
describe('Client-Server', function () {
  ...
  describe('API', function () {
    ...
    it('should issue DELETE /messages/44 ', function () {
      ...
      let callback = sinon.spy();
      Babble.deleteMessage('44', callback);
      server.respond();
      sinon.assert.calledWith(callback, true);
    });
    it('should issue ERROR /moshe ', function () {
      server.respondWith('POST', `${apiUrl}/moshe`, JSON.stringify({error: 400}));
      let callback = sinon.spy();
      Babble.test('POST',apiUrl+'/moshe','', callback);
      server.respond();
